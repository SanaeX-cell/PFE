...
if (isset($_POST['delete_id']) && is_numeric($_POST['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM contributeurs WHERE id = ?");
    $stmt->bind_param("i", $del_id);
    $stmt->execute();
    $stmt->close();
    header('Location: contributeurs.php');
    exit();
}

// ========== RÉCUPÉRATION DES CONTRIBUTEURS ==========
$sql = "SELECT * FROM contributeurs ORDER BY date_inscription DESC";
$result = mysqli_query($conn, $sql);
$tous_contributeurs = [];
while ($row = mysqli_fetch_assoc($result)) {
    $tous_contributeurs[] = $row;
}
$total_contributeurs = count($tous_contributeurs);
...
<title>ConnectAid - Gestion des contributeurs</title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .contrib-table { width: 100%; border-collapse: collapse; margin-top: 4px; border-radius: 12px; overflow: hidden; }
  .contrib-table thead tr { background: linear-gradient(90deg, #F47B20 0%, #E06A10 100%); }
  .contrib-table th { text-align: left; font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.9); text-transform: uppercase; letter-spacing: 0.6px; padding: 12px 10px; }
  .contrib-table td { padding: 13px 10px; border-bottom: 1px solid var(--border); vertical-align: middle; font-size: 13px; color: var(--text-primary); }
...
<div class="modal-overlay" id="deleteModal">
  <div class="modal">
    ...
    <h3>Supprimer le contributeur</h3>
...
<script>
const allContributeurs = <?php echo json_encode($tous_contributeurs); ?>;
const ITEMS_PER_PAGE = 10;
let currentPage = 1;
let filteredContributeurs = [...allContributeurs];
...
function renderContributeurs(data, page) {
  ...
  if (slice.length === 0) {
    ...
    slice.forEach(c => {
      ...
      row.innerHTML = `
        <td><div class="contrib-name"><div class="contrib-avatar">${initials(c.nom, c.prenom)}</div>${escapeHtml(c.nom)}</div></td>
        <td>${escapeHtml(c.prenom)}</td>
        <td>${escapeHtml(c.email)}</td>
        <td>${escapeHtml(c.telephone)}</td>
        <td>${formatDateFr(c.date_inscription)}</td>
        <td>
          <button class="btn-delete" onclick="openModal(${c.id})">
...
function goToPage(page) {
  currentPage = page;
  renderContributeurs(filteredContributeurs, currentPage);
}
